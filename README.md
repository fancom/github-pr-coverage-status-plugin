...
* In ```pom.xml```, set the versions of:
  * the plugin itself
  * the dependencies
  * the Jenkins instance to create
* Run ```mvn hpi:run``` to start a local Jenkins instance with the plugin and all the dependencies installed
* Access [http://localhost:8080/jenkins/](http://localhost:8080/jenkins/) to test the plugin

### Build Plugin

* Run ```mvn clean package``` to build the plugin's package into ```target/*.hpi``` file

### Install Plugin

* To install the plugin, upload it to the [Plugin Manager](https://cicd.fancom.io/pluginManager/advanced)
